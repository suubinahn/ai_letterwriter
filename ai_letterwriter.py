...
st.write("'새해 인사'를 전하고 싶지만 어떤 말을 전해야 좋을지 고민인 당신! AI_레터라이터를 통해 특별한 새해 편지를 작성해 보세요! 떠오르는 사람의 이름과 나이 그리고 그 대상을 표현하기 좋은 간단한 정보만 입력해 주면 끝! 최대 단어 수, 생성할 편지 수, 편지 내용의 성격을 결정지을 MBTI 유형, 작성 모드 그리고 내용에 포함시키고자 하는 키워드까지 더해주면 더욱 특별한 편지가 생성된답니다.")
...
간결하게 작성해주세요.
이모지를 적절하게 섞어주세요.
mbti가 주어질 경우, 해당 mbti 키워드는 편지 내용에 언급하지 말고 선택된 mbti의 성격만을 고려하여 작성해주세요.
